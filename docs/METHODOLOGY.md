...
| the trie's **animation** shows exactly the char-steps the benchmark counts, for search, insert *and* delete — and does so over the *Rust corpus's own* keys and probes, so the picture is chained to the bench twin's counts rather than to a fresh hand-written list. The same test proves the tracer perturbs no counter (traced and untraced runs agree) | `src/viz/trace.trie.test.ts` (keys/probes from `conformance/corpus-str.txt`) | none (op-counts) |
